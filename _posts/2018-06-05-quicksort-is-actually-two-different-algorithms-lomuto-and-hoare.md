...
QuickSort is a [Divide & Conquer](https://en.wikipedia.org/wiki/Divide_and_conquer_algorithm) sorting algorithm, which immediately hints how you should start writing it:
    ...
    where T : IComparable<T>
{
    if (low < high)
    {
        ...
        // recursive calls, item at pi will not be moved further
        QuickSortLomuto(items, low, pi - 1);
        QuickSortLomuto(items, pi + 1, high);
    }
}
